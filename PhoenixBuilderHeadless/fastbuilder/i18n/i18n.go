...
func SelectLanguage() {
	...
	curLangDict := make(map[uint16]string)
	{
		...
		for lang := range LangDict {
			...
			bridge_fmt.Printf("[%d] %s\n", i, LangDict[lang][LanguageName])
			i++
		}
	}
	reader := bufio.NewReader(os.Stdin)
	for {
		bridge_fmt.Printf("(ID): ") // No \n
	...
	if file, err := os.Create(config); err != nil {
		bridge_fmt.Println("Error creating language config file: %v", err)
		bridge_fmt.Println("Error ignored.")
	} else {
		_, err = file.WriteString(SelectedLanguage)
		if err != nil {
			bridge_fmt.Println("Error saving language config: %v", err)
			bridge_fmt.Println("Error ignored.")
		}
		file.Close()
	}
...
func Init() {
	...
	langdict, aru := LangDict[SelectedLanguage]
	if !aru {
		bridge_fmt.Printf("Ordered language doesn't exist.\nPlease reselect one:\n")
		SelectLanguage()
		langdict, aru = LangDict[SelectedLanguage]
		if !aru {
